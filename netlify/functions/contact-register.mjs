...
const validate = (value) => {
  ...
  const keys = new Set(['name', 'company', 'email', 'phone', 'service', 'message', 'consent', 'website']);
  if (Object.keys(value).some((key) => !keys.has(key))) return { ok: false };
  const input = {
    name: clean(value.name, 100), company: clean(value.company, 120), email: clean(value.email, 160).toLowerCase(),
    phone: clean(value.phone, 24), service: clean(value.service, 160), message: clean(value.message, 1200),
    consent: value.consent === true, website: clean(value.website, 200)
  };
  const valid = input.name.length >= 3 && input.company.length >= 2 && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(input.email)
    && /^\+?[0-9][0-9\s-]{8,20}[0-9]$/.test(input.phone) && input.service && input.message.length >= 10 && input.consent && !input.website;
...
const sendToMake = async ({ input, eventId, fetchImpl = global.fetch }) => {
  ...
  try {
    const response = await fetchImpl(url, {
      ...
      body: JSON.stringify({ event_id: eventId, event: 'project_inquiry', source: 'dupixelcode.com/landing', received_at: new Date().toISOString(), Nombre: input.name, Empresa: input.company, Email: input.email, WhatsApp: input.phone, Servicio_Interes: input.service, Mensaje: input.message, Acepta_Privacidad: true })
    });
...
export default async function handler(request) {
  ...
  const eventId = `contact_${randomUUID()}`;
  try {
    await sendToMake({ input: parsed.input, eventId });
    console.info('[CONTACT] Inquiry accepted', { event_id: eventId, timestamp: new Date().toISOString() });
    return json(202, { accepted: true, event_id: eventId });
  } catch (error) {
    console.error('[CONTACT] Inquiry delivery failed', { event_id: eventId, reason: error instanceof Error ? error.message : 'unknown_error', timestamp: new Date().toISOString() });
    return json(503, { error: 'No fue posible enviar la solicitud. Intenta de nuevo o contáctanos por WhatsApp.' });
  }
}

export const config = { path: '/api/contact-register', method: 'POST', rateLimit: { action: 'rate_limit', windowLimit: 5, windowSize: 900, aggregateBy: ['ip', 'domain'] } };
